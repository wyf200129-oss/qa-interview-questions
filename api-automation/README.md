...
## Q43. 幂等性怎么测试？结合你的ERP项目具体讲一下

### 问题描述

面试官问了概念题"什么是幂等性"之后，大概率会追问"那你怎么测的？"——他要的不是定义背诵，是**你在真实项目里怎么设计幂等性测试用例**，有没有踩过并发幂等的坑。

> 这道题直接决定面试官对你"测试设计能力"的评价——只会概念=零分，能说出具体业务场景和验证SQL=满分。

---

### 一、核心概念回顾（一句话）

> **同一个操作，执行一次和执行多次，结果完全一样。** 在接口层面：同一个请求（相同参数），无论发多少次，服务端的数据状态不会有重复的副作用。
...
| 业务场景 | 幂等问题 | 如果失效的后果 |
|:---|:---|:---|
| 采购入库单审核 | 用户双击"审核"按钮 | 预算扣两次、库存加两次 |
| 财务凭证生成 | 月底批量任务跑了两次 | 凭证重复，财务对账炸了 |
| 库存调拨 A→B 100件 | 网络重试导致执行两次 | A仓库库存变负数 |
| 销售出库确认 | 两个操作员同时点确认 | 库存扣两次，实际只出了一次货 |

---

### 三、面试回答脚本（约2.5分钟，结合ERP项目）

> "幂等性测试我在ERP项目里主要针对**审批类接口**和**库存变更类接口**来做，这两类一旦重复执行，前者状态错乱，后者直接导致数据不一致、财务对账都对不上。
>
> 我的测试方法分三步：
>
> **第一步，单接口重复提交**：同一个单据号+相同参数，连续发两次请求。验证第二次返回幂等提示（'单据已审核，不重复操作'），不是500错误也不是再次执行了业务逻辑。然后查数据库——只有一条业务变更记录。
>
> **第二步，并发请求模拟**：用Python多线程同时发多个相同请求，模拟网络重试或者多人同时操作。这是最贴近生产的——因为幂等bug大多数不是串行重试触发的，是并发竞态条件触发的。
>
> 比如库存调拨场景：A仓库200件→调拨100件到B仓库。我用两个线程同时发调拨请求，验证最终A仓库=100件、B仓库=100件，没有出现A仓库变成0或者变成负数的异常。
>
> **第三步，批量任务的幂等**：ERP里有月底结账这种批量生成凭证的任务。我在测试环境手动触发两次，用SQL查：
>
> `SELECT business_id, COUNT(*) FROM vouchers GROUP BY business_id HAVING COUNT(*) > 1`
>
> 必须返回空——同一笔业务单据只能生成一张凭证。"
...
**业务**：采购入库单从"待审核"→"已审核"，审核后扣减采购预算、更新库存。
...
def approve_order():
    resp = requests.post("https://xxx/api/purchase/approve",
                         json={"orderId": "CG-20240601-001"})
    print(resp.json())

# 并发发送两个审核请求
t1 = threading.Thread(target=approve_order)
t2 = threading.Thread(target=approve_order)
t1.start(); t2.start()
t1.join(); t2.join()
```

**验证点**：
1. 第二次请求返回 "单据已审核，不重复操作"
2. 数据库里审核状态只变了一次
3. 预算只扣减了一次
4. 库存只更新了一次
...
**业务**：从A仓库调拨100件商品到B仓库。接口：A减100、B加100。

**测试操作**：
```
准备数据：A仓库库存200件，B仓库0件，调拨单号DB-20240601-001

连发两次调拨执行请求（相同调拨单号+相同参数）：
  第一次 → A:200→100, B:0→100 ✅
  第二次 → 被幂等拦截，返回"调拨单已执行" ✅
```

**验证点**：
1. A仓库库存 = 100（不是0，没有减两次）
2. B仓库库存 = 100（不是200，没有加两次）
3. 数据库调拨单状态 = "已执行"，执行次数 = 1

**并发测试**：10个线程同时发相同调拨请求 → 最终只有一笔执行成功

---

#### 场景三：销售出库确认

**业务**：操作员点击"确认出库"，扣库存+更新销售单状态。

**测试操作**：
```
模拟两个仓库操作员在各自电脑上同时对同一张销售单点了"确认出库"
→ 请求几乎同时到达服务端
```

**关键设计**：请求头里加 `requestId`（客户端生成的UUID）
```
Header: X-Request-Id: a1b2c3d4-...
服务端用这个ID做幂等判断：
  相同requestId + 相同销售单号 → 重试，幂等返回
  不同requestId + 相同销售单号 → 两个不同操作员的请求，都该处理（但只有一个能成功）
```
...
**业务**：月底结账时，系统批量生成财务凭证。同一笔业务单据只能对应一张凭证。

**测试操作**：
```
手动在数据库里准备10笔待生成凭证的业务单据
连调两次批量生成接口
```
...
**额外测试——部分失败后的重试**：
```
假如100笔单据，生成到第50笔时服务挂了
重启后重新执行 → 前50笔不能重复生成 → 最终总数=100笔（不是150笔）
```
...
批量重试 → 批量任务手动连跑两次
     ↓
异常恢复 → 执行中途服务挂了，重启后不重复
```

| 测试层次 | 测什么 | 怎么触发 |
|:---|:---|:---|
| 串行重复 | 幂等逻辑本身是否正确 | 相同参数连续发两次 |
| 并发竞态 | 高并发下幂等锁是否有效 | 多线程同时发10个请求 |
| 批量重试 | 批量任务幂等 | 手动连跑两次批量接口 |
| 异常恢复 | 中断后重试是否幂等 | 执行到一半kill服务→重启→再执行 |
...
| 验证项 | 具体检查 | 通过标准 |
|:---|:---|:---|
| 接口响应 | 第二次请求返回的业务code | 正确的幂等提示（如"已处理"），不是500/异常 |
| 数据库状态 | 单据状态字段 | 只变更了一次 |
| 关联数据 | 预算/库存/凭证等关联记录 | 只产生了一条，无重复 |
| SQL检查 | `GROUP BY ... HAVING COUNT(*) > 1` | 返回空 |
| 并发安全 | 10个线程同时发的最终结果 | 只有一个成功，9个幂等返回 |

---

**关键词**：幂等性测试 / 并发竞态 / 库存调拨 / 重复提交 / 凭证去重 / 唯一业务键 / requestId
